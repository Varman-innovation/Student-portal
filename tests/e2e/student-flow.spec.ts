...
test("new student completes the full conversion journey", async ({ page }, testInfo) => {
  ...
  await page.getByLabel("Mobile number").fill(testInfo.project.name === "mobile-chrome" ? "9876543210" : "9876543211");
  ...
  await page.getByLabel("Region").selectOption("Tamil Nadu");
  await page.getByLabel("Language").selectOption("Tamil");
  await page.getByRole("button", { name: /continue/i }).click();

  await page.getByLabel("Degree").selectOption("B.Tech / B.E");
  await page.getByLabel("Branch").selectOption("Computer Science and Engineering (CSE)");
  ...
  await expect(page.getByText("Entrepreneurship Masterclass")).toBeVisible();
  await page.getByRole("button", { name: /register for webinar/i }).click();
...
test("onboarding validates required fields and resumes at step two", async ({ page }, testInfo) => {
  ...
  await page.getByRole("button", { name: /continue/i }).click();
  await expect(page.getByText("Education details", { exact: true }).first()).toBeVisible();
  await page.reload();
